 ...
 const [loading,setLoading]=useState(false)
  ...
  const onCreateNewProjectPressed = async () => {
    ...
    setLoading(true)
          ...
          const result = await axios.post("http://3.132.112.94:4000/api/update", {
    ...
    setLoading(true)
           ...
           let res = await convertHtmlApi(response.data.filepath, response.data.project_name);
          console.log("Convert to html:", res);
    setLoading(false)

          history.push('/edit-html', { fileName: response.data.filepath, fileNameTemaplate: response.data.project_name })
        }
          }
...
_tempArray.push({data_id:updatedTextFrames[targetIndex].data_id,newText:updatedTextFrames[targetIndex].contents})
             ...
             <div className="row">
               ...
               {projectList.map((item, i) => (
                 <ProductCard
                   ...
                   settingHandler={settingHandler}
                   changeStatusHandler={changeStatusHandler}
                   projectData={[item.image, item.created_at, item.project_name, item.creator_first_name, item.creator_last_name]}
                 />
                 // <div
                 //   id={item.id}
                 //   // onClick={(e) =>
